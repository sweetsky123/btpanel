                    ...
                    for attack_type, stats in attack_stats.items():
                        if stats['count'] > 0:
                            ...
                            for c, t, ip, url in items[:5]:
                                ...
                                if len(u) > 200:
                                    u = u[:200] + '...'
                                lines.append(f"时间{disp_t}，{ip}尝试进行{c}次{pattern_info['name']}，请求URL为{u}")
                            info_text = "\r\n".join(lines) if lines else f"检测到{pattern_info['name']}攻击，共{stats['count']}次尝试"
                            result.append({
                                "name": f"{webinfo['name']} 检测到{pattern_info['name']}",
                                "info": info_text if info_text else f"检测到{pattern_info['name']}攻击，共{stats['count']}次尝试",
