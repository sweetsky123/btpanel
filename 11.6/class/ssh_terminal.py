...
class ssh_host_admin(ssh_terminal):
    ...
    def set_ai_shell_status(args):
        ...
        bt_agent_status = plu_obj.get_soft_find(public.to_dict_obj({'sName':'bt_agent'}))
        if not bt_agent_status['status']:
            return public.returnMsg(False, "未安装AI助手插件，请先安装后再开启该功能")
        if public.version_to_tuple(bt_agent_status['version']) < (1,2,0):
            return public.returnMsg(False, "请将AI助手插件更新到1.2.0或以上版本后再开启该功能")
