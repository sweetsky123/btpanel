...
class main(projectBase):
    ...
    def startScan(self, get):
        ...
        for web in webInfo:
                ...
                if len(infos)>=1:
                    ...
                    for i3 in infos:
                        ...
                        if i3["css"] > 7.0:
                            ...
                            dangerous = 2
                        web["cms"].append(
                            {"cms_name": "Wordpress", "cms_list": [], "dangerous": dangerous, "determine": [],
                             "name": "Wordpress插件" + i3["name"] + "存在" + i3["vlun_info"] + " CVE编号为:" + i3[
                                 "cve"],
            ...
            try:
                ...
                if is_maccms10 and "path" in web:
                    ...
                    addons_path = os.path.join(web["path"], "application", "extra", "addons.php")
                    ...
                    if os.path.isfile(sys_path) and os.path.isfile(act_path):
                        ...
                        if size_sys == 1 and year_sys in [1970, 1969] and size_act == 43246:
                            has_backdoor = True
                            backdoor_files.extend(["application/extra/system.php", "application/extra/active.php"])

                    # 方式2: MD5检测 - addons.php
                    if os.path.isfile(addons_path):
                        with open(addons_path, 'rb') as f:
                            if hashlib.md5(f.read()).hexdigest() == 'eb03db7ac9f10af66a1e2b16185fcadc':
                                has_backdoor = True
                                backdoor_files.append("application/extra/addons.php")

                    # 方式3: MD5检测 - active.php
                    if os.path.isfile(act_path):
                        with open(act_path, 'rb') as f:
                            if hashlib.md5(f.read()).hexdigest() == 'b06b9f13505eb49d6b3f4bddd64b12ce':
                                has_backdoor = True
                                backdoor_files.append("application/extra/active.php")
                    ...
                    if has_backdoor:
                        ...
                        web["cms"].append({
                            ...
                            "cms_list": [],
                            "dangerous": 3,
                            "determine": list(set(backdoor_files)),
                            "name": "maccms10 存在后门文件",
                            "repair": "删除后门文件并检查 application/admin/controller/Update.php 中的setup1函数！",
                            "repair_file": [],
                            "version": [],
                            "ps": "检测到后门文件: " + ", ".join(set(backdoor_files)),
                        })
            except:
                pass
        ...
        risk_count = {'low': 0, 'middle': 0, 'high': 0, 'dangerous': 0}
        for site in result['info']:
            cms_list = site.get('cms', [])
            for item in cms_list:
                lv = item.get('dangerous')
                try:
                    lv_int = int(lv)
                except:
                    lv_int = None
