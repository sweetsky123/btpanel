...
# -------------------------------------------------------------------
import os, ast, datetime, json, importlib, inspect, random, shutil, base64, threading
import time
import zipfile
import tarfile
...
- 通用辅助：代码编写、知识解答、文本处理等。
        ...
        "max_context_tokens": 131072,         #总上下文大小限制（默认256k），用于前端展示和上下文判断，不传递给OpenAI接口
    ...
    def get_config(self, get):
        ...
        data = {}
        if self.config.get('api_key') == self.DEFAULT_CONFIG['api_key']:
            api_usage_url = self.config['api_usage_url']
            if 'bt.cn' in api_usage_url:
                api_usage_url = public.get_home_node(api_usage_url)
            response = requests.get(api_usage_url, headers=self.config['default_headers'])
            res = response.json()
        ...
        configs = {
            ...
            "daily_quota": {
                ...
                "expire_at": data.get("expire_at", 50)
            },
            "config": self.config,
    ...
    def get_models(self, get):
        ...
        if not base_url or not key:
            ...
            if 'bt.cn' in base_url:
                base_url = public.get_home_node(base_url)
        ...
        try:
            ...
            if "aliyun" in public.get_oem_name() and "bt.cn" in base_url:
                for i in range(len(model_list)-1, -1, -1):
                    ...
                    if model_id.startswith("doubao") or model_id in ["glm-4-7-251222", "deepseek-v3-2-251201", "deepseek-r1-250528", "kimi-k2-thinking-251104"]:
                        model_list.pop(i)

            models_data = [ dict(model) for model in model_list]
            
            # 同步模型配置到config.model_config中
            self._sync_model_config_to_storage(models_data)

            return public.return_data(True, data=models_data)
        except Exception as e:
            return public.return_data(True, data=[])

    def _sync_model_config_to_storage(self, models_data):
        """
        内部方法：将模型数据同步到 config 中的 model_config
        """
        try:
            existing_model_config = self.config.get('model_config', {})
            ...
            for model_data in models_data:
                ...
                # 直接写入，不判断是否已有自定义配置
                existing_model_config[model_id] = model_data
            
            self.config['model_config'] = existing_model_config
            self._save_config()
        except:
            # 静默失败，不影响主流程
            pass

    def _get_model_max_context_tokens(self, model_name):
        """
        获取指定模型的最大上下文 token 数
        优先使用模型特定配置，否则使用全局默认配置
        """
        max_context_tokens = int(self.config.get("max_context_tokens", 131072))
        
        if model_name:
            model_config = self.config.get("model_config", {})
            if model_name in model_config:
                model_specific_max = model_config[model_name].get("max_context_tokens")
                if model_specific_max:
                    max_context_tokens = int(model_specific_max)
    ...
    def get_tool_list(self, get):
        ...
        if tool_type == 'aics':
            aics_tool_ids = ['Glob', 'Grep', 'LS', 'Read', 'Write', 'StopCommand', 'CheckCommandStatus', 'RunCommand', 'WebFetch',"get_system_resources","get_top_processes","get_sites","get_site_analysis","get_server_ip","get_site_overview","get_sites_logs","add_site","delete_site"]
    ...
    def chat(self, get):
        ...
        # 工具合并策略：smartMode 为 1 时启用全部工具，否则请求中的 tools 优先，追加模板配置中的 tools（去重）
        if smart_mode == '1':
            # smartMode 为 1 时，启用全部工具
            final_tools = [meta["id"] for name, meta in registry._metadata.items() if meta.get("show", True)]
        else:
            # 正常模式：请求中的 tools 优先，追加模板配置中的 tools（去重）
        ...
        agent_config = {
            ...
            "max_context_tokens": self._get_model_max_context_tokens(model),
        ...
        try:
            ...
            for chunk in agent.chat(user_input):
                if chunk.get("type") == "content":
                    ...
                    yield self.sse_pack(event="meta_info", data={"user_msg_id": current_user_id, "ai_msg_id": current_ai_id, "last_loop_tokens": last_loop_tokens})
                else:
    ...
    def _generate_session_title(self, session_dir, user_text):
        """异步生成会话标题并写入 meta.json"""
        try:
            ...
            result = single_agent.chat(
                prompt="你是一个标题生成助手。请根据用户输入生成一个简短、准确的聊天标题（不超过20个字）。只返回标题，不要其他内容。",
            ...
            try:
                ...
                for session_id in dirs:
                    ...
                    try:
                        ...
                        # 优先使用 AI 复写的标题，如果没有则使用第一句 content
    ...
    def get_chat(self, get):
        ...
        try:
            ...
            max_context_tokens = int(self.config.get("max_context_tokens", 262144))
            ...
            return public.return_data(True, data={
                ...
                "meta": {
                    ...
                    "context_usage_ratio": round((total_tokens / max_context_tokens), 4) if max_context_tokens > 0 else 0
                }
            })
        except Exception as e:
    ...
    def compact_chat(self, get):
        """
        手动上下文压缩接口
        委托给 ContextCompactor 执行完整压缩流程
        """
        session_id = get.get('session_id')
        if not session_id:
            return public.returnMsg(False, "缺少参数 session_id")
        ...
        compact_config = ContextCompactor.load_compact_config(prompts_dir=os.path.join(self.plugin_path, 'prompts'))
        preserve_rounds = int(get.get('preserve_rounds', 0))
        ...
        compactor_config = {
            ...
            "preserve_rounds": preserve_rounds if preserve_rounds > 0 else compact_config.get('preserve_rounds', 3),
        ...
        result = compactor.compact_session(
            session_id=session_id,
            sessions_dir=os.path.join(self.data_path, sessions_dir),
            preserve_rounds=preserve_rounds if preserve_rounds > 0 else None
        )

        compactor.close()

        if result['success']:
            return public.return_data(True, data=result['data'])
        return public.returnMsg(False, result['error'])
