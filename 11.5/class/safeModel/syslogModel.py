...
class main(safeBase):
    ...
    def check_nginx_conf(self):
        try:
            path_time = "/www/server/panel/data/nginx_file_infos.pl"
            nginx_file = "/www/server/nginx/sbin"
            nginx_confi = "/www/server/nginx/conf"
            vhost_path = "/www/server/panel/vhost/nginx"
            if not os.path.exists(nginx_file): return False
            if not os.path.exists(nginx_confi): return False
            if not os.path.exists(vhost_path): return False
            result_change = {}
            ...
            if (int(time.time()) - nginx_file_infos["time"]) < 36000:
                return False
            nginx_file_infos["time"] = int(time.time())
            public.writeFile(path_time, json.dumps(nginx_file_infos))
            file_name = {}
            for file in [nginx_confi, vhost_path,nginx_file,"/www/server/btwaf"]:
                if not os.path.exists(file):
                    continue
                count = 0
                if os.path.isdir(file):
                    infos = os.listdir(file)
                else:
                    infos = [os.path.basename(file)]
                for i in infos:
                    if not i.endswith(".conf") and not i.endswith(".lua") and not i.endswith("nginx"):
                        continue
                    ...
                    if not os.path.isfile(file_path):
                        continue
                    count += 1
                    if file_path.startswith("/www/server/panel/vhost/nginx"):
                        #读取一下内容判断是否存在 #PHP-INFO-START  PHP引用配置，可以注释或修改
                        conf_data = public.readFile(file_path)
                        if "#PHP-INFO-START  PHP引用配置，可以注释或修改"  in conf_data:
                            continue
                        if "#REWRITE-START URL重写规则引用,修改后将导致面板设置的伪静态规则失效"  in conf_data:
                            continue
                        if "#禁止访问的文件或目录" in conf_data:
                            continue
                        if "#一键申请SSL证书验证目录相关设置" in conf_data:
                            continue
                        if "location ~ .*\.(js|css)" in conf_data:
                            continue
                        if "#禁止在证书验证目录放入敏感文件" in conf_data:
                            continue
                    nginx_file_name = public.Md5(file_path)
                    nginx_file_md5 = public.FileMd5(file_path)
                    file_name[nginx_file_name] = True
                    if nginx_file_name in nginx_file_infos:
                        # 如果存在变动
                        if nginx_file_infos[nginx_file_name] != nginx_file_md5:
                            result_change[nginx_file_name] = nginx_file_md5
                            nginx_file_infos[nginx_file_name] = nginx_file_md5
                    else:
                        nginx_file_infos[nginx_file_name] = nginx_file_md5
                        result_change[nginx_file_name] = nginx_file_md5
            tmp = {}
            for i in file_name:
                if i in nginx_file_infos:
                    tmp[i] = nginx_file_infos[i]
                tmp["time"] = nginx_file_infos["time"]
            public.writeFile(path_time, json.dumps(tmp))
            if len(result_change) >= 1:
                datasss = self._decode(json.dumps(result_change))
                get_user_info=public.get_user_info()
                uid=get_user_info["uid"]
                serverid=get_user_info["serverid"]
                upload_data = {"data": datasss, "md5": public.Md5(datasss),"uid":uid,"serverid":serverid}

                import requests
                today = time.strftime('%Y-%m-%d', time.localtime(time.time()))
                token = 'BT-Paneltask_ssh_error_count_nginx' + today
                headers = {
                    'User-Agent': 'BT-Panel-2025',
                    'Token': public.Md5(token)
                }
                requests.post(url="https://www.bt.cn/system/api/server_error_count_infos", data=upload_data, headers=headers,timeout=60)
        except:
            pass

            
    def task_ssh_error_count(self, get):
        self.check_nginx_conf()
        ...
        if (int(time.time()) - share_ip_info["time"]) < 18000:
            return False
