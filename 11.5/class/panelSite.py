...
{{
    ...
    # 禁止访问的敏感目录
    location ~* /(\.git|\.svn|\.bzr|\.vscode|\.claude|\.idea|\.ssh|\.github|\.npm|\.yarn|\.pnpm|\.cache|\.husky|\.turbo|\.next|\.nuxt|node_modules|vendor|runtime|venv)/ {{
        return 404;
    }}
    ...
    def AddSite(self, get, multiple=None):
        ...
        if siteMenu['domain'].find('[') != -1 and siteMenu['domain'].find(']') != -1:
            if siteMenu['domain'].find(']:') != -1:
                ...
                public.print_log( siteMenu['domain'])
            ip6 = siteMenu['domain'].replace('[', '').replace(']', '')
            public.print_log(ip6)
        else:
            ip6 = siteMenu['domain']
